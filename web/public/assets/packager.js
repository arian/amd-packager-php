...
var Packager = this.Packager = {
	...
	init: function(form){
		...
		form.getElements('.options input[type="radio"]').each(function(element){
			element.setStyle('display', 'none');

			var parent = element.getParent('tr'), name = element.get('name');

			(radios[name] || (radios[name] = [])).push({radio: element, parent: parent});

			parent.addListener('click', function(){
				radios[name].each(function(radio){
					if (element == radio.radio){
						radio.radio.set('checked', true);
						radio.parent.addClass('checked').addClass('selected');
					} else {
						radio.radio.set('checked', false);
						radio.parent.removeClass('checked').removeClass('selected');
					}
				});
			});
